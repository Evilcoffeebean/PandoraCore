 ...
 */
public class RubyCommand extends CommandBase {

    private DatabaseManager manager;

    public RubyCommand(DatabaseManager manager) {
        super("ruby", "/ruby <player> <amount>", new Rank[]{Rank.DEV}, 2);
        ...
        if (cmd.getName().equalsIgnoreCase(getCommand())) {
            if (caller instanceof Player) {
                ...
                try {
                    ...
                    manager.setRubies((Player)target, amount);
                    UtilLogger.help(caller, "Coin", "You set " + target.getName() + "'s ruby amount to " + ChatColor.YELLOW + amount);
                } catch (Exception e) {
