...
public class UnbanCommand extends CommandBase {
    ...
    public UnbanCommand(DatabaseManager manager) {
        super("unban", new Rank[]{Rank.DEV}, 2);
        ...
        if (cmd.getName().equalsIgnoreCase(getCommand())) {
            if (caller instanceof Player) {
                ...
                if (args.length < getLength()) {
                    UtilLogger.help(p, "Command Center", "Usage: /unban <player> <reason>");
                    return true;
                }
                ...
                if (args[1] != null) {
                    ...
                    if (table.isBanned(UUID)) {
                        table.unpunishUser(UUID, PunishType.BAN);
                        ...
                        UtilLogger.help(p, "Punish", "That player isn't banned.");
                        return true;
                    }
                }
            } else {
                if (args.length < getLength()) {
                    UtilLogger.log("Command Center", "Usage: /unban <player> <reason>");
                    return true;
                }
                final String UUID = Bukkit.getOfflinePlayer(args[0]).getUniqueId().toString();
                if (args[1] != null) {
